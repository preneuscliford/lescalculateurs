...
function detectType(content) {
  ...
  if (lower.includes('impot') || lower.includes('ir-calculator')) return 'IMPOT';
  if (lower.includes('notaire') || lower.includes('frais-notaire')) return 'NOTAIRE';
  if (lower.includes('rsa')) return 'RSA';
  if (lower.includes('prime-activite')) return 'PRIME';
  if (lower.includes('salaire')) return 'SALAIRE';
  if (lower.includes('pret') || lower.includes('credit-immobilier')) return 'PRET';
...
function verbalizeMontants(content) {
  ...
  const patterns = [
    ...
    {
      regex: /(<(strong|span|div)[^>]*class="[^"]*result[^"]*"[^>]*>[^<]*)(\d{2,4})\s*€/gi,
      replace: '$1<span class="result-verbal">environ $3 €</span><span class="result-exact" style="display:none;">$3 €</span>'
    },
    // Pattern: Montant affiche dans texte "297 €"
    {
      regex: /(>)(\d{2,4})(\s*€\s*<)/gi,
      replace: '$1<span class="result-verbal">environ $2 €</span><span class="result-exact" style="display:none;">$2 €</span>$3'
    }
  ];
