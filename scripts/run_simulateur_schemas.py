...
def detect_type(content, filename):
    ...
    for key, sim_type in SIMULATEUR_TYPES.items():
...
def generate_howto(sim_type):
    ...
    name = names.get(sim_type, f'Comment utiliser le simulateur ?')
...
def is_simulateur(file):
    try:
        ...
        for key in SIMULATEUR_TYPES.keys():
            if key in filename:
                return True
        return False
    except:
...
with REPORT_CSV.open('w', newline='', encoding='utf-8') as f:
    writer = csv.DictWriter(f, fieldnames=['fichier', 'howto_added', 'breadcrumb_added', 'internal_links_added', 'status'])
    writer.writeheader()
