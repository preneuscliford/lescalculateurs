...
function loadCfg() {
  ...
  if (fs.existsSync(genP)) {
    try {
      ...
      const dmOverrides =
        b?.notaire?.droitsMutation?.overrides ||
        b?.notaire?.droits_mutation?.ancien?.overrides ||
        {};
...
function computeDroits(code, price, cfg, type) {
  ...
  const dmtoPct =
    cfg.dmto && cfg.dmto[code] != null
      ? Number(cfg.dmto[code]) / 100
      : cfg.dmto_default || 0.0632;
...
function computeCsi(price, cfg) {
  ...
  const min =
    cfg && cfg.csi && cfg.csi.minimum != null ? Number(cfg.csi.minimum) : 15;
...
function updateDmtoText(html, code, cfg) {
  const departmental =
    cfg.dmto_departmental &&
    cfg.dmto_departmental[code] &&
    Number(cfg.dmto_departmental[code].taux);
    ...
    if (perDept && perDept.url) {
      ...
      sourceLinks = Array.isArray(cfg.sources)
        ? cfg.sources
            .map(
              (u) =>
                `<a href="${u}" class="text-blue-600 hover:underline" rel="nofollow noopener" target="_blank">${u}</a>`,
            )
            .join(" • ")
