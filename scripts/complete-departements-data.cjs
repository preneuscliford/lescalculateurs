...
tousLesDeptos.forEach((dept) => {
  if (!deptData[dept]) {
    ...
    console.log(
      `✅ Ajout ${dept}: ${deptData[dept].nom.padEnd(30)} | Taux: ${(
        deptData[dept].tauxDroits * 100
      ).toFixed(1)}% | Cadastre: ${deptData[dept].fraisDivers.cadastre}€`
    );
    added++;
  } else {
    existing++;
