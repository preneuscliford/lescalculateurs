...
/**
 * Retourne la liste des départements manquants à créer
 */
...
/**
 * Crée une page département minimale avec structure HTML et <article>
 */
...
<html lang="fr">
  <head>
    ...
    <meta name="description" content="Calculez vos frais de notaire 2025 en ${name} (${code}). Simulation rapide et gratuite : ancien vs neuf, droits, émoluments, formalités, CSI et TVA." />
  </head>
  <body class="bg-gray-50">
    <article class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <p class="text-gray-700 mb-4">Données indicatives basées sur le barème officiel 2025.</p>
...
/**
 * Point d'entrée: crée les pages manquantes et affiche un résumé
 */
function main() {
  const missing = getMissingDepartments();
  if (missing.length === 0) {
    console.log('Aucune page manquante à créer.');
    return;
  }
  const created = missing.map(createDepartmentPage);
  console.log(`Pages créées: ${created.length}`);
