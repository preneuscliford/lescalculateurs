...
const globalConfig = JSON.parse(
  fs.readFileSync(path.join(__dirname, '../data/global-monitoring.json'), 'utf8')
);

console.log('\n╔════════════════════════════════════════════════════════════════╗');
console.log('║  🌍 VÉRIFICATION GLOBALE - Tous les Calculateurs               ║');
console.log('╚════════════════════════════════════════════════════════════════╝\n');
...
console.log(`   Prochain audit complet: ${globalConfig.summary.next_full_audit}\n`);

// Display each calculator
console.log('╔════════════════════════════════════════════════════════════════╗');
console.log('║  📋 ÉTAT DE CHAQUE CALCULATEUR                                 ║');
console.log('╚════════════════════════════════════════════════════════════════╝\n');
...
globalConfig.calculators.forEach((calc, idx) => {
  ...
  sources.forEach(key => {
    ...
    console.log(`        Prochain check: ${source.next_check} (${daysUntil} jours) ${urgency}`);
...
// Calendar view
console.log('\n╔════════════════════════════════════════════════════════════════╗');
console.log('║  📅 CALENDRIER GLOBAL DE VÉRIFICATION                          ║');
console.log('╚════════════════════════════════════════════════════════════════╝\n');
...
// Verification gaps
console.log('\n╔════════════════════════════════════════════════════════════════╗');
console.log('║  ⚠️  CALCULATEURS NÉCESSITANT VÉRIFICATION                      ║');
console.log('╚════════════════════════════════════════════════════════════════╝\n');

const needsVerification = globalConfig.calculators.filter(c => c.status.includes('À vérifier'));
needsVerification.forEach((calc) => {
  console.log(`\n❌ ${calc.name}`);
  
  const dataSources = Object.keys(calc.data_sources);
  const nextChecks = dataSources.map(key => new Date(calc.data_sources[key].next_check));
...
console.log('📧 Suggestions: configurer rappel mail pour chaque date importante\n');
