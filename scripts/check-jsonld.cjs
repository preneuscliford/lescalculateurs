...
const BASE = path.resolve(process.cwd(), 'src', 'pages', 'blog', 'departements');
...
if (!FIX) {
  console.log('Run with --fix to attempt automatic repairs (backs up files to ./reports/jsonld-backups-<stamp>/)');
  errors.slice(0, 30).forEach(e => console.log(`${e.file} [block ${e.index}]: ${e.message}`));
  process.exit(1);
}

// --fix mode
const stamp = nowStamp();
const backupDir = path.resolve(process.cwd(), 'reports', `jsonld-backups-${stamp}`);
...
for (const name of files) {
  ...
  const blocks = extractJsonLdBlocks(html);
  let changed = false;
  const newHtml = html.replace(/(<script[^>]*type=["']application\/ld\+json["'][^>]*>)([\s\S]*?)(<\/script>)/gi, (m, open, content, close) => {
    try {
      JSON.parse(content);
      return m; // valid
    } catch (err) {
