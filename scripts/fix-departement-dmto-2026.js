...
const DMTO_2026 = {
  ...
  "976": 5.09, // Mayotte
  // Taux standard 5.80%
  "05": 5.80, "06": 5.80, "07": 5.80, "16": 5.80, "26": 5.80, "27": 5.80,
  "48": 5.80, "60": 5.80, "65": 5.80, "71": 5.80, "971": 5.80, "972": 5.80
...
function computeFraisNotaire(code, price, type) {
  ...
  return { droits, emoluments, debours, formalites, csi, tva, total: droits + emoluments + debours + formalites + csi + tva, dmtoRate }
}
...
function processFile(filePath) {
  ...
  const code = match[1].toUpperCase() === '2A' ? '2A' : (match[1].toUpperCase() === '2B' ? '2B' : match[1])
  ...
  if (html !== original) {
    ...
    console.log(`✓ ${base}: DMTO ${newAncienPct} (ancien) / ${newNeufPct} (neuf)`)
    return true
  }
  return false
}
...
function main() {
  ...
  const files = fs.readdirSync(dir).filter(f => f.startsWith('frais-notaire-') && f.endsWith('.html'))
