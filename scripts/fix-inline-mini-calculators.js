...
function fixFile(filePath) {
  ...
  // 3) Debours/formalites: utiliser fraisDiversParDepartement pour l\'ancien
  out = out.replace(
  ...
  // 5) Droits: base sur dmto par departement (ancien) et notaire.droitsMutation.neuf (neuf). Suppression doublons.
  out = out.replace(
  ...
  // 6) Afficher le taux DMTO utilise sous le pourcentage
  out = out.replace(
    /\+\s*'<div class="text-xs text-gray-600 mt-1">'\s*\+\s*d\.pourcentage\.toFixed\(2\)\s*\+\s*'% du prix<\/div>'/,
    (m) => {
      const insert = " + '<div class=\"text-xs text-gray-600 mt-1\">' + d.pourcentage.toFixed(2) + '% du prix</div>' + '<div class=\"text-xs text-gray-600\">Taux DMTO utilise: ' + (Math.floor((d.tauxDroits * 100) * 100)/100).toFixed(2).replace('.', ',') + '%</div>'";
      return insert
    }
  )
  if (out !== html) {
...
function main() {
  ...
  console.log(`Mini-calculateur: ${changed} fichier(s) mis a jour sur ${files.length}.`)
}
