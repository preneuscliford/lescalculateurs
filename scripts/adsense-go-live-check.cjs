...
if (!fs.existsSync(adsPath)) {
  ...
  if (!/google\.com,\s*pub-/i.test(ads)) {
    fail(
      "public/ads.txt ne contient pas de publisher Google valide (google.com, pub-...)",
    );
  } else {
...
if (!fs.existsSync(loaderPath)) {
  ...
  if (missing.length) {
    fail(
      `third-party-loader incomplet, éléments manquants: ${missing.join(", ")}`,
    );
  } else {
    pass(
      "third-party-loader contient les garde-fous consentement (denied par défaut + banner fallback)",
    );
  }
}
...
if (missingLoader.length) {
  fail(
    `Pages sans /third-party-loader.js (${missingLoader.length}):\n- ${missingLoader.slice(0, 20).join("\n- ")}${missingLoader.length > 20 ? "\n- ..." : ""}`,
  );
} else {
  pass(
    `Toutes les pages HTML complètes ciblées (${pageFiles.length}) passent par /third-party-loader.js`,
  );
}

if (directIncludes.length) {
  fail(
    `Pages avec script tiers direct détecté (${directIncludes.length}):\n- ${directIncludes.slice(0, 20).join("\n- ")}${directIncludes.length > 20 ? "\n- ..." : ""}`,
  );
} else {
  pass(
    "Aucun chargement direct GTM/GA/AdSense trouvé dans src/pages et src/simulateurs (hors backups)",
  );
}
