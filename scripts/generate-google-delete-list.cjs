...
validUrls.forEach(url => {
  ...
  if (!url.endsWith('/')) {
    ...
    const urlApexIndex = url.replace('https://www.', 'https://') + '/index.html';
    urlsToDelete.add(urlApexIndex);
  }
});

// Créer le fichier de suppression
const deleteList = Array.from(urlsToDelete)
  .sort()
  .join('\n');

const outputPath = path.resolve(__dirname, '..', 'urls-to-delete-from-google.txt');
...
console.log('💡 ASTUCE: Vous pouvez aussi utiliser l\'outil "Remappage d\'URL" si');
console.log('   disponible pour mapper automatiquement les anciennes → nouvelles URLs\n');
