...
.map(
 (h, idx) =>
 `<th class=\"p-3 text-center font-semibold\">${h}<br><button class="text-xs text-red-600 hover: text-red-800 mt-1" onclick="deleteImpotScénario(${idx})">✕ Supprimer</button></th>`
)
.join("")}</tr></thead>
 <tbody>
...
function deleteImpotScénario(index: number) {
 compIR.splice(index, 1);
 renderIR();
}
(window as any).deleteImpotScénario = deleteImpotScénario;
