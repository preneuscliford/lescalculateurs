...
const comparisonStorage = {
 getScénarios(): ComparisonScénario[] {
 const stored = localStorage.getItem("impot-scenarios");
 return stored ? JSON.parse(stored) : [];
 },
 addScénario(scenario: ComparisonScénario) {
 const scenarios = this.getScénarios();
 scenarios.push(scenario);
 localStorage.setItem("impot-scenarios", JSON.stringify(scenarios));
 },
 removeScénario(id: string) {
 const scenarios = this.getScénarios().filter((s) => s.id !== id);
 localStorage.setItem("impot-scenarios", JSON.stringify(scenarios));
 },
};
...
const impotConfig = {
 ...
 const allCases: ComparisonScénario[] = autoCases.map((c, i) => ({
 id: `auto-${i}`,
 name: c.name,
 revenu: d.revenu,
 parts: c.parts,
 ...
 const customScénarios = comparisonStorage.getScénarios();
 allCases.push(...customScénarios);
...
function initComparisonUI(casesWithIR: any[], currentData: any) {
 ...
 showScénarioModal(currentData.revenu);
 });
 }
 ...
 if (id) {
 comparisonStorage.removeScénario(id);
 location.reload();
 }
 });
 });
...
function showScénarioModal(currentRevenu: number) {
...
.value;
 ...
 const scenario: ComparisonScénario = {
 id: `custom-${Date.now()}`,
 name,
 revenu,
 parts,
 ...
 comparisonStorage.addScénario(scenario);
 location.reload();
 });
