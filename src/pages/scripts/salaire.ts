...
.map(
 (h, idx) =>
 `<th class=\"p-3 text-center font-semibold\">${h}<br><button class="text-xs text-red-600 hover: text-red-800 mt-1" onclick="deleteSalaireScénario(${idx})">✕ Supprimer</button></th>`
)
.join("")}</tr></thead>
 <tbody>
...
function deleteSalaireScénario(index: number) {
 compState.splice(index, 1);
 renderSalaireComparaison();
}
(window as any).deleteSalaireScénario = deleteSalaireScénario;
