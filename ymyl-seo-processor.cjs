...
const getYMYLRiskLevel = (content, filename) => {
  const highRiskKeywords = ['apl', 'rsa', 'impot', 'aah', 'are', 'asf', 'prime-activite', 'alloc', 'caf', 'aide'];
  const medRiskKeywords = ['pret', 'notaire', 'plus-value', 'taxe', 'salaire'];
  
  const lowerFilename = filename.toLowerCase();
  const lowerContent = content.toLowerCase();
  
  if (highRiskKeywords.some(k => lowerFilename.includes(k) || lowerContent.includes(k))) {
    return 'HIGH';
  }
  if (medRiskKeywords.some(k => lowerFilename.includes(k) || lowerContent.includes(k))) {
    return 'MED';
  }
  return 'LOW';
};
...
const generateTitle = (oldTitle, filename, h1Text) => {
  ...
  keyword = keyword.charAt(0).toUpperCase() + keyword.slice(0, 40).toLowerCase();
...
const generateMetaDesc = (title, filename) => {
  const cta = 'Calculez gratuitement votre estimation. Outil officiel et gratuit.';
...
const getInternalLinks = (filename) => {
  ...
  } else if (filename.includes('notaire') || filename.includes('frais-notaire')) {
...
const processFile = (filePath, relativePath) => {
  try {
    ...
    // Compter avant
    numbersBefore = (content.match(moneyRegex) || []).length + 
    ...
    const newH1 = h1Text ? `${h1Text} 2026` : path.basename(filename, '.html').replace(/-/g, ' ');
    processedContent = processedContent.replace(
      /<h1[^>]*>[^<]*<\/h1>/i,
      `<h1>${newH1}</h1>`
    );
    ...
    numbersAfter = (processedContent.match(moneyRegex) || []).length + 
    ...
    const h1Opt = 'O';
    const schemaAdded = 'OUI';
    const actionMain = ymyLevel === 'HIGH' ? 'Verbalisation forte + Alertes multiples' : 
                       ymyLevel === 'MED' ? 'Verbalisation modérée + Alerte tableau' : 
