...
function detectType(content) {
  ...
  if (lower.includes('apl') || lower.includes('aide-logement') || lower.includes('caf.fr')) return 'APL';
  if (lower.includes('impot') || lower.includes('ir-') || lower.includes('impots.gouv')) return 'IMPOT';
  if (lower.includes('notaire') || lower.includes('frais-notaire') || lower.includes('notaires.fr')) return 'NOTAIRE';
  ...
  if (lower.includes('pret') || lower.includes('credit-immobilier') || lower.includes('mensualite')) return 'PRET';
  if (lower.includes('taxe-fonciere') || lower.includes('taxe habitation')) return 'TAXE';
  if (lower.includes('plus-value') || lower.includes('plusvalue')) return 'PLUSVALUE';
  if (lower.includes('ik') || lower.includes('kilometrique') || lower.includes('0,502')) return 'IK';
  return 'AUTRE';
}

// Verifier si schema FAQPage existe
function hasFAQSchema(content) {
  return content.includes('"@type": "FAQPage"') || 
         content.includes('"@type":"FAQPage"') ||
         (content.includes('FAQPage') && content.includes('application/ld+json'));
}
